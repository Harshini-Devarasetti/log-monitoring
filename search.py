...
err_df = pd.DataFrame(columns=['Filename', 'Timestamp', 'Type', 'Line No.', 'Issue Details'])
...
def process_log():
    ...
    regex_pattern = r'(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2})' + r'(% s)' % '|'.join(pattern)
            ...
            if pathlib.Path(item).suffix in log_scan_types:
                ...
                with open(item, 'r', encoding="ISO-8859-1") as file:
                    ...
                    for line in file:
                        ...
                        if match:
                            err_lst = [str(item), match.group(1), match.group(2), str(line_cnt), line]
                            print(err_lst)
